...
本项目基于 PyPTO 实现运行在 Ascend NPU 上的 DeepSeek V4 Flash 单卡 BF16 推理版本，包括模型 Kernel、整模 prefill/decode 编排、权重加载、专家缓存和 Device Runtime。
...
| 文档 | 内容 |
|---|---|
| [Models 实现总览](docs/models/README.md) | 模型结构、BF16 计算路径、运行约束及各 Kernel 专题文档入口 |
| [Serving 实现总览](docs/serving/README.md) | 整模 prefill/decode 编排、权重与状态管理、Device Runtime 及验证方法 |
| [文本生成](docs/serving/01_generate.md) | `generate.py` 参数、prompt 编码、采样循环和完整生成流程 |
| [整模冒烟验证](docs/serving/02_smoke_model.md) | `smoke_model.py` 参数、裁剪执行方式和验收方法 |
| [专家缓存导出](docs/serving/03_export_expert_cache.md) | `export_expert_cache.py` 参数、packed BF16 格式和导出流程 |
