...
def _make_official_compressor(args) -> torch.nn.Module:
    ...
    with official_model.set_dtype(torch.bfloat16):
        compressor = official_model.Compressor(args, compress_ratio=128, head_dim=args.head_dim)

    with torch.no_grad():
        compressor.ape.copy_(torch.randn(128, args.head_dim) * 0.02)
        compressor.wkv.weight.copy_(
            (torch.randn(args.head_dim, args.dim, dtype=torch.bfloat16) * 0.02).float()
        )
    ...
    compressor.kv_cache = torch.zeros(1, args.max_seq_len // 128, args.head_dim, dtype=torch.bfloat16)
...
def _compressor_tensors(compressor: torch.nn.Module, x: torch.Tensor, args) -> dict[str, torch.Tensor]:
    ...
    freqs_cis = compressor.freqs_cis[:cutoff:128]
...
def test_golden_compressor_ratio128_prefill_matches_official_model(tiny_ratio128_args, seq_len: int) -> None:
    ...
    x = (torch.randn(1, seq_len, tiny_ratio128_args.dim, dtype=torch.float32) * 0.1).to(torch.bfloat16)

    expected_return = compressor(x.clone(), start_pos=0)
    tensors = _compressor_tensors(compressor, x, tiny_ratio128_args)
...
def test_golden_compressor_ratio128_decode_matches_official_model(tiny_ratio128_args, start_pos: int) -> None:
    compressor = _make_official_compressor(tiny_ratio128_args)
    x = (torch.randn(1, 1, tiny_ratio128_args.dim, dtype=torch.float32) * 0.1).to(torch.bfloat16)
    with torch.no_grad():
        ...
        compressor.kv_cache.copy_(
            (torch.randn_like(compressor.kv_cache.float()) * 0.1).to(torch.bfloat16)
        )

    tensors = _compressor_decode_tensors(compressor, x, tiny_ratio128_args, start_pos)
    ...
    torch.testing.assert_close(tensors["kv_state_out"], compressor.kv_state, rtol=0, atol=0)
    torch.testing.assert_close(tensors["score_state_out"], compressor.score_state, rtol=0, atol=0)
    torch.testing.assert_close(tensors["compressed_cache_out"], compressor.kv_cache, rtol=0, atol=0)
    if expected_return is None:
        torch.testing.assert_close(tensors["compressed"], torch.zeros_like(tensors["compressed"]), rtol=0, atol=0)
    else:
