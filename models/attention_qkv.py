...
def attention_qkv_fwd(
    ...
    q_flat = pl.reshape(q_unflat, [tokens, N_HEADS * HEAD_DIM])
    q_pre_rope_flat = pl.reshape(q, [tokens, N_HEADS * HEAD_DIM])
    ...
    for tb in pl.range(token_blocks):
        ...
        for h in pl.spmd(N_HEADS, name_hint="q_scale"):
            h0 = h * HEAD_DIM
            ...
            for db in pl.range(Q_HEAD_D_BLOCKS):
                ...
                for row in pl.range(valid_tok):
                    ...
                    q_pre_rope_flat = pl.assemble(q_pre_rope_flat, out_row, [t0 + row, d0])

    q_pre_rope = pl.reshape(q_pre_rope_flat, [B, tokens, N_HEADS, HEAD_DIM])
    q = rope_4d_512_fwd(q_pre_rope, cos, sin, q)
