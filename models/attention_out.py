...
def attention_out_fwd(
    ...
    """Compute official attention output projection after inverse RoPE."""
    o.bind_dynamic(1, S_DYN)
    proj.bind_dynamic(1, S_DYN)
    out.bind_dynamic(1, S_DYN)

    tokens = pl.tensor.dim(o, 1)
    o_flat = pl.reshape(o, [tokens, N_HEADS * HEAD_DIM])
...
def attention_out_fwd_test(
    ...
    proj, out = attention_out_fwd(o, wo_a_t, wo_b_t, proj, out)
    return proj, out
...
def golden_attention_out(tensors):
    ...
    o = tensors["o"].view(B, tensors["o"].shape[1], O_GROUPS, O_GROUP_IN)
